...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($enteredOtp == $_SESSION['reset_otp'] && time() < $_SESSION['otp_expiry']) {
        ...
        $stmt = $conn->prepare("UPDATE students_registrtaion SET password=? WHERE email=?");
        $stmt->bind_param("ss", $hashed, $email);
        if ($stmt->execute()) {
            unset($_SESSION['reset_email'], $_SESSION['reset_password'], $_SESSION['reset_otp'], $_SESSION['otp_expiry']);
            header("Location: studentLogin.php?reset=success");
            exit();
        } else {
...
<body>
  <div class="otp-box">
    <a href="forgotPassword.php" style="float:left; text-decoration:none; color:#2575fc;">← Back</a>
    <h1>Verify OTP</h1>
